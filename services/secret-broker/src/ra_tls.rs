//! RA-TLS attestation verification - native secret-broker implementation.
//!
//! This module is secret-broker-owned code in the isolated copy. It does NOT
//! path-dep into `../dstack/` or any vendored Phala source tree. The only
    ...
    impl Attestation {
        ...
        pub async fn verify_with_ra_pubkey(
            ...
            let pccs_url = pccs_url
                ...
                .ok_or_else(|| {
                    anyhow!("SECRET_BROKER_PCCS_URL must be supplied for RA-TLS verification")
                })?;
